...
export const DEPENSES: DepenseRecord[] = [
  { id: 'dep_001', patientId: 'pat_001', date: '2026-02-20', montant: '12 000 XOF', categorie: 'Consultation', description: 'Consultation cardiologie urgence', payeur: 'Patient' },
  { id: 'dep_002', patientId: 'pat_001', date: '2026-02-22', montant: '8 500 XOF',  categorie: 'Laboratoire',  description: 'Analyse biologique complète',    payeur: 'Assurance' },
  { id: 'dep_003', patientId: 'pat_001', date: '2026-01-10', montant: '45 000 XOF', categorie: 'Hospitalisation', description: 'Hospitalisation 5 jours',      payeur: 'Assurance' },
  { id: 'dep_004', patientId: 'pat_001', date: '2026-03-01', montant: '6 200 XOF',  categorie: 'Pharmacie',    description: 'Médicaments ordonnance mars',     payeur: 'Patient' },
];

/* ══════════════════════════════════════════════════════════════
   SECTION 13 — FONCTIONS UTILITAIRES
   À remplacer par des appels API. Signature identique conservée.
══════════════════════════════════════════════════════════════ */

/** Récupère le dossier patient par ID */
...
export const getAnalysesForPatient = (patientId: string): AnalyseRecord[] =>
  ANALYSES.filter((a) => a.patientId === patientId);

/** Examens d'un patient */
export const getExamensForPatient = (patientId: string): ExamenPatient[] =>
  EXAMENS_PATIENT.filter((e) => e.patientId === patientId);
...
export const getTraitementsForPatient = (patientId: string): TraitementPatient[] =>
  TRAITEMENTS_PATIENT.filter((t) => t.patientId === patientId);

/** Rendez-vous d'un patient */
export const getRendezVousForPatient = (patientId: string): RendezVous[] =>
  RENDEZ_VOUS.filter((r) => r.patientId === patientId);

/** Mesures de santé d'un patient */
export const getMesuresForPatient = (patientId: string): MesureSante[] =>
  MESURES_SANTE.filter((m) => m.patientId === patientId);

/** Dépenses d'un patient */
export const getDepensesForPatient = (patientId: string): DepenseRecord[] =>
...
/** Médecin par ID */
export const getMedecinById = (id: string): MedecinRecord | undefined =>
  MEDECINS.find((m) => m.id === id);
